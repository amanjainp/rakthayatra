...
import { useHospitalDashboardData } from '../../hooks/useDashboardData';
import { Building, Plus, Search, Calendar, Landmark } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useAuth } from '../../hooks/useAuth';

export const HospitalDashboard: React.FC = () => {
  const { user } = useAuth();
  const { data, isLoading } = useHospitalDashboardData();

  const quickActions = [
    { label: 'Request Blood Units', href: '/requests', desc: 'File emergency requirements for compatible blood units.', icon: <Plus className="w-5 h-5" /> },
    { label: 'Search Inventory', href: '/requests', desc: 'Check compatible inventories and reserves across banks.', icon: <Search className="w-5 h-5" /> },
    { label: 'Sponsor Donation Camp', href: '/camps', desc: 'Associate with camps to boost local supplies.', icon: <Calendar className="w-5 h-5" /> },
    { label: 'Facility Profile', href: '/profile', desc: 'Settle address mappings and registry license updates.', icon: <Landmark className="w-5 h-5" /> },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-lg font-bold text-white font-display">Logistics Directory</h3>
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-white font-display">Active Transfusions</h3>
